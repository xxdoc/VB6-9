...
StringToSend = "fortypoundhead.com"
...
If (lAddress <> -1) And (lAddress <> 0) Then
    ...
    Else
        Post frmMain.txtOutput, "failure opening icmp handle."
        ping = -1
    End If
Else
    ping = -1
...
Public Function DoPing(strAddressToPing As String) As Long
    ...
    If SocketsInitialize() Then
        ...
        'Address to ping
        strIPAddress = strAddressToPing

        'Ping the IP that is passing the address and get a reply.
        lngSuccess = ping(strIPAddress, 100, Reply)
...
End Function
Public Function ResolveAndPing(strHostName As String) As String
    ...
    If Len(strIPAddress) > 0 Then
        ...
        ResolveAndPing = strHostName & " [" & strIPAddress & "] " & strPingResult
        
    Else
        
        ResolveAndPing = "unable to resolve " & strHostName
